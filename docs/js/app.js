...
$(document).ready(function() {
  ...
  cl2
    .on('initialized.owl.carousel', function () {cl2.find(".owl-item").eq(0).addClass("current");})
    .owlCarousel({
    items : slidesPerPage,
    dots: false,
    nav: false,
  ...
  function syncPosition(el) {
    ...
    cl2
      ...
      .eq(current)
      .addClass("current");
    var onscreen = cl2.find('.owl-item.active').length - 1;
    var start = cl2.find('.owl-item.active').first().index();
    var end = cl2.find('.owl-item.active').last().index();
    if (current > end) {cl2.data('owl.carousel').to(current, 100, true);}
    if (current < start) {cl2.data('owl.carousel').to(current - onscreen, 100, true);}
  }
  function syncPosition2(el) {if(syncedSecondary) {var number = el.item.index;cl1.data('owl.carousel').to(number, 100, true);}}
  cl2.on("click", ".owl-item", function(e){e.preventDefault();var number = $(this).index();cl1.data('owl.carousel').to(number, 300, true);});
